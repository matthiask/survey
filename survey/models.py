...
class Survey(models.Model):
    ...
    code = models.CharField(
        ...
        default=lambda: get_random_string(length=20))
...
class QuestionGroup(models.Model):
    survey = models.ForeignKey(
        Survey, related_name='groups', verbose_name=_('survey'))
...
class SurveyAnswer(models.Model):
    ...
    survey = models.ForeignKey(
        Survey, related_name='answers', verbose_name=_('survey'))
    code = models.CharField(
        _('code'), max_length=20,
        default=lambda: get_random_string(length=20))
    status = models.PositiveIntegerField(
        _('status'), choices=STATUS_CHOICES, default=INITIAL)
    ...
    visitor_contact = models.CharField(
        _('contact'), max_length=100)

    visitor_counter = models.PositiveIntegerField(
        _('visitor counter'), default=0)

    conductor_company = models.CharField(
        _('company'), max_length=100, blank=True)
    conductor_name = models.CharField(
        _('name'), max_length=100, blank=True)
    conductor_contact = models.CharField(
        _('contact'), max_length=100, blank=True)
    ...
    def __unicode__(self):
        return u'Survey %s answered by %s' % (
    ...
    def details(self):
        ...
        questions = Question.objects.filter(
            group__survey=self.survey,
        ).order_by('group', 'ordering').select_related('group')
